  ...
  context.subscriptions.push(
    vscode.commands.registerTextEditorCommand("br-lsp.nextOccurrence", (editor) => {
      ...
      if (cursorWordRange !== undefined) {
        ...
        const followingText: string = editor.document.getText(
          editor.selection.with(editor.selection.end, editor.document.lineAt(editor.document.lineCount - 1).range.end),
        );
        ...
        if (nextMatchIndex >= 0) {
          ...
          editor.selection = new vscode.Selection(
            ...
            editor.document.positionAt(matchOffset + selectedText.length),
          );
        }
  ...
  context.subscriptions.push(
    vscode.commands.registerTextEditorCommand("br-lsp.prevOccurrence", (editor) => {
      ...
      if (cursorWordRange !== undefined) {
        editor.selection = new vscode.Selection(cursorWordRange.start, cursorWordRange.end);
        const selectedText: string = editor.document.getText(editor.selection);
        const precedingText: string = editor.document.getText(
        ...
        while ((precedingMatch = newSearch.exec(precedingText)) != null) {
          editor.selection = new vscode.Selection(
            editor.document.positionAt(precedingMatch.index),
            editor.document.positionAt(precedingMatch.index + selectedText.length),
          );
        }
